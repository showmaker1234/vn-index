...
function tierMethod(){
  ...
  return '<details class="ranking-method" id="strength-method"><summary>评分标准 3.0：模拟方法、公式、权重与局限</summary><div>'
    ...
    +'<li><b>六项指标</b>：主要贡献 = 对两种敌人的期望伤害占敌方生命的比例，与治疗量 × '+f.healFactor+' 取较高者（同一职业只担一个定位；舞娘的「跳舞」按满额计）；承伤 = 能承受的敌方攻击次数（对数）；个人与团队技能 = 站内七维模型的技能评分；机动 = 职业移动加成 + 飞行 + 凯伊线骑乘；可用场次 = 第一部 '+Math.round(a.campaignShare.p1*100)+'%、第二部 '+Math.round(a.campaignShare.p2*100)+'%、第三部 '+Math.round(a.campaignShare.p3*100)+'% 中能参战的比例，<b>第一部入队越晚扣得越多</b>（实际入队章 = 出场章与「3 + 0.5 × 所需名声」章取较晚者）；招募成本 = 支援、名声、金钱、道具、外传、委托要求折算的扣分。</li></ul>'
...
function showStrengthCharacter(id){
  ...
  const barNote=b?{
    ...
    availability:'能参战的场次约占全流程 '+b.availability+'%；'+(b.joinPart===1?'第一部第 '+tierFmt(b.joinChapter)+' 章入队':'第'+['','一','二','三'][b.joinPart]+'部加入')+'。',
